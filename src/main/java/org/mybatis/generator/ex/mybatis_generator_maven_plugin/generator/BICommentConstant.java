...
public class BICommentConstant {
	...
	static {
		...
		// 8个基本方法
		// =======================
		String[] countByExample = { "countByExample:通过Example计算总数", "@param example 查询条件",
				"@return 总条数" };
		String[] deleteByExample = { "deleteByExample:通过Example删除", "@param example 刪除条件",
				"@return 受影响条数" };
		String[] deleteByPrimaryKey = { "deleteByPrimaryKey:通过主键删除", "@param id 表的主键",
				"@return 受影响条数" };
		String[] insertSelective = { "insertSelective:有选择性的插入数据(空数据不会插入)", "@param record 表对象",
				"@return 受影响条数" };
		String[] selectByExample = { "selectByExample:通过Example查询条件集合", "@param example 查询条件",
				"@return 对象集合" };
		String[] selectByPrimaryKey = { "selectByPrimaryKey:通过主键查询单个对象", "@param id 单表的主键",
				"@return 表对象" };
		...
		// 扩展方法-批量插入
		// =======================
		String[] insertBatch = { "insertBatch 批量插入数据(按集合中第一条数据的有效列)", "@param records 数据集合",
				"@return 受影响条数" };
...
//				"@return 表对象Map集合 (key为主键，value为表对象)" };
		...
		// 扩展方法-指定列查询
		// =======================
		String[] insertIfAbsent = { "insertIfAbsent 如果不存在，则插入",
				"@param record 表对象 -【对象数据中，必须包含判断存在的标准(唯一性约束)】", "@return 受影响条数" };

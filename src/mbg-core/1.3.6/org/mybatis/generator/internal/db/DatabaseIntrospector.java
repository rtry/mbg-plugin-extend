		...
		for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns.entrySet()) {
			for (IntrospectedColumn introspectedColumn : entry.getValue()) {
				...
				if (isTrue(tc.getProperty(PropertyRegistry.TABLE_USE_ACTUAL_COLUMN_NAMES))) {
					...
					introspectedColumn.setJavaProperty(getCamelCaseString(calculatedColumnName,
							false));
				}
				...
				if (fullyQualifiedJavaType != null) {
					...
					if (warn) {
						...
						String warning = getString("Warning.14", //$NON-NLS-1$
								Integer.toString(introspectedColumn.getJdbcType()), entry.getKey()
...
//		}
	...
	private Map<ActualTableName, List<IntrospectedColumn>> getColumns(TableConfiguration tc)
		...
		boolean delimitIdentifiers = tc.isDelimitIdentifiers()
				|| stringContainsSpace(tc.getCatalog()) || stringContainsSpace(tc.getSchema())
		...
		// ================================================
		// rs 代表information_schema 中cloums中的数据
		// ================================================
		while (rs.next()) {
			...
			if (supportsIsGeneratedColumn) {
				introspectedColumn
						.setGeneratedColumn("YES".equals(rs.getString("IS_GENERATEDCOLUMN"))); //$NON-NLS-1$ //$NON-NLS-2$
			}
		...
		if (answer.size() > 1 && !stringContainsSQLWildcard(localSchema)
			...
			ActualTableName inputAtn = new ActualTableName(tc.getCatalog(), tc.getSchema(),
					tc.getTableName());
	...
	private List<IntrospectedTable> calculateIntrospectedTables(TableConfiguration tc,
		...
		for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns.entrySet()) {
			...
			IntrospectedTable introspectedTable = ObjectFactory.createIntrospectedTable(tc, table,
					context);
	...
	private void enhanceIntrospectedTable(IntrospectedTable introspectedTable) {
		try {
			...
			ResultSet rs = databaseMetaData.getTables(fqt.getIntrospectedCatalog(),
					fqt.getIntrospectedSchema(), fqt.getIntrospectedTableName(), null);
			if (rs.next()) {
